...
class NeuralNetwork:
    ...
    def sigmoid(self, x, shift):
        ...
        v = W0 + self.remove_shift(W1 * x, shift) - self.remove_shift(
            W3 * self.remove_shift(x * self.remove_shift(x * x, shift), shift), shift)
        return v
    ...
    def init_model(self, config, no_features):
        ...
        for (j, i), w_j_i in zip(product(range(self.no_neurons), range(no_features)),
                                 (np.random.rand(no_features * self.no_neurons) - 0.5)):
    ...
    def train(self, config, dataset, weights):
        ...
        for epoch in range(config['epochs']):
            ...
            for sample_idx in range(len(dataset)):
                ...
                for j in range(len(self.w_0)):
                    dz_0[j] = self.remove_shift(da_0[j] * self.remove_shift(a_0[j] * (1 - a_0[j]), config['precision']),
                                                config['precision'])
                ...
                for i in range(len(self.w_1[0])):
                    self.w_1[0][i] -= self.remove_shift(self.add_shift(config['lr'], config['precision']) * dw_1[0][i],
                                                        config['precision'])
                self.b_1[0] -= self.remove_shift(self.add_shift(config['lr'], config['precision']) * db_1[0],
                                                 config['precision'])

                for j in range(len(self.w_0)):
                    for i in range(len(self.w_0[j])):
                        self.w_0[j][i] -= self.remove_shift(
                            self.add_shift(config['lr'], config['precision']) * dw_0[j][i], config['precision'])
                    self.b_0[j] -= self.remove_shift(self.add_shift(config['lr'], config['precision']) * db_0[j],
                                                     config['precision'])
            print(
                f'[{epoch:>2}] loss: {loss / len(dataset) / config["precision"]:.5f} acc: {self.score(dataset, config):.2f}')