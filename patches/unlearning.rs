    ...
    match options.variant {
        ...
        ProofVariant::NIZK => {
            ...
            ***************************/
            ...
            println!("[DEBUG] Preparing to serialize R1CS...");

            match to_string_pretty(&prover_data.r1cs) {
                Ok(r1cs_json) => {
                    let mut file = File::create("circuit.r1cs.json").unwrap();
                    file.write_all(r1cs_json.as_bytes()).unwrap();
                    println!("[+] R1CS constraints saved to circuit.r1cs.json");
                }
                Err(e) => {
                    println!("[ERROR] Failed to serialize R1CS: {:?}", e);
                }
            }
